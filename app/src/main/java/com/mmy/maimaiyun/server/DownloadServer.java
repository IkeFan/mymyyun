package com.mmy.maimaiyun.server;
        ...
        public void run() {
            ...
            try {
                ...
                while ((read = is.read(buffer)) != -1 && !isStop) {
                    ...
                    if (mListener != null)
                        mHandler.post(()->mListener.onProgressUpdate((int) ((mCurrentProgress * 1.0f) / length * 100)));
                    //                    Log.d("lucas", "当前下载进度:" + mCurrentProgress);
                }
                conn.disconnect();
                if (mListener != null)
                    mHandler.post(()->mListener.onComplete(apkFile));
            } catch (IOException e) {
                e.printStackTrace();
                if (mListener != null)
