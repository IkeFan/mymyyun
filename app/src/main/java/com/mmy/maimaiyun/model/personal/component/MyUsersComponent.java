package com.mmy.maimaiyun.model.personal.component;

import com.mmy.maimaiyun.AppComponent;
import com.mmy.maimaiyun.base.scoped.ActivityScoped;
import com.mmy.maimaiyun.model.personal.module.MyUsersModule;
import com.mmy.maimaiyun.model.personal.ui.activity.MyUsersActivity;

import dagger.Component;

/**
 * @创建者 lucas
 * @创建时间 2017/9/20 0020 18:15
 * @描述 TODO
 */
@ActivityScoped
@Component(modules = MyUsersModule.class,dependencies = AppComponent.class)
public interface MyUsersComponent {
    void inject(MyUsersActivity activity);
}
